...
<div class="cart-items">
	<div class="container">
			 <h3 class="tittle">My Inactive items(<%=items %>)</h3>
	...
	%> 
				...
				<script>$(document).ready(function(c) {
					$('<%="."+close%>').on('click', function(c){
						if (confirmComplete(0)){
						$('<%="."+cart%>').fadeOut('slow', function(c){
							
							$('<%="."+cart%>').remove();
							window.location.replace("RemoveItem?id="+<%=result.getInt("idItem")%>);
				...
				<script>$(document).ready(function(c) {
					$('<%="."+edit%>').on('click', function(c){
						
							window.location.replace("ChangeItem?id="+<%=result.getInt("idItem")%>);
